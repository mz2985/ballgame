...
class Ball(GameObject):
    def __init__(self, canvas, x, y):
        self.direction = [1, -1]
        self.speed = 15#弹球速度，通过改变速度改变游戏难度
        item = canvas.create_oval(x, y, x + 20, y + 20, fill='white')
        super().__init__(canvas, item)
...
class Game(tk.Frame):
    def __init__(self, master):
        ...
        # 设置画板并放置
        self.canvas = tk.Canvas(self, bg='#242424', width=self.width, height=self.height)
        self.canvas.pack()
        self.pack()
    ...
    def setup_game(self):
        ...
        self.text = self.canvas.create_text(400, 200, text='单击鼠标左键开始游戏', font=('Helvetica', 36), fill='#FCFCFC')
        # 将鼠标左键单击与开始游戏绑定在一起
    ...
    def update_lives_text(self):
        ...
        if self.lives_text is None:
            self.lives_text = self.canvas.create_text(60, 30, text=text, font=('Helvetica', 16), fill='#8B0000')
        else:
    ...
    def update_scores_text(self):
        ...
        if self.scores_text is None:
            self.scores_text = self.canvas.create_text(60, 60, text=text, font=('Helvetica', 16), fill='#8B0000')
        else:
